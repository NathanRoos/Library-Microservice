...
API_FIELDS["http://localhost:8080/api/v1/accounts"]="firstname lastname email"
API_FIELDS["http://localhost:8080/api/v1/books"]="title"
API_FIELDS["http://localhost:8080/api/v1/workers"]="firstname lastname"
API_FIELDS["http://localhost:8080/api/v1/accounts/84a8ec6e-2fdc-4c6d-940f-9b2274d44420/loans"]="loanStatus"
...
ID_KEY_FROM_URL() {
  ...
  [[ "$endpoint" == "workers" ]] && echo "librarianId" || echo "${endpoint%s}Id"
}
...
randomize_fields() {
  ...
  for field in "${fields[@]}"; do
    ...
    elif [[ "$field" == "loanStatus" ]]; then
      # Use correct ENUM values from LoanStatusEnum
      local enum_values=("PENDING" "ACTIVE" "COMPLETED" "OVERDUE")
      value="${enum_values[$RANDOM % ${#enum_values[@]}]}"
    else
