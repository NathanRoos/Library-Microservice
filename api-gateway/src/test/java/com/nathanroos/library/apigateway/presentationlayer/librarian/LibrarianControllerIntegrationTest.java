...
class LibrarianControllerIntegrationTest {
    ...
    void testGetLibrarianById_Success() throws Exception {
        LibrarianResponseModel mockResponse = LibrarianResponseModel.builder()
                .librarianId("lib-123")
                .firstname("John")
                .lastname("Doe")
        ...
        when(librarianService.getLibrarian("lib-123")).thenReturn(mockResponse);

        mockMvc.perform(get("/api/v1/workers/lib-123")
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.librarianId").value("lib-123"));
    }

    @Test
    void testGetLibrarianById_NotFound() throws Exception {
        when(librarianService.getLibrarian("lib-999"))
                .thenThrow(new NotFoundException("Librarian not found"));

        mockMvc.perform(get("/api/v1/workers/lib-999"))
                .andExpect(status().isNotFound());
    }

    @Test
    void testGetAllLibrarians() throws Exception {
        when(librarianService.getworkers()).thenReturn(
                List.of(LibrarianResponseModel.builder()
                        .librarianId("lib-123")
                        .firstname("John")
                        .lastname("Doe")
                        .email("john.doe@example.com")
        ...
        mockMvc.perform(get("/api/v1/workers"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].librarianId").value("lib-123"));
    }

    @Test
    void testAddLibrarian() throws Exception {
        ...
        when(librarianService.createLibrarian(any(LibrarianRequestModel.class))).thenReturn(response);

        mockMvc.perform(post("/api/v1/workers")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.librarianId").value("lib-456"));
    }
    ...
    void testUpdateLibrarian() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/workers/lib-123")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.librarianId").value("lib-123"));
    ...
    void testDeleteLibrarian() throws Exception {
        mockMvc.perform(delete("/api/v1/workers/lib-123"))
